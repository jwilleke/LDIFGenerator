...
public class LDIFGeneration
{
    ...
    private static String outPutFilename = System.getProperty("user.home") + System.getProperty("file.separator") + "ldifgen.ldif";
    ...
    // dataPath, baseDN, outPutFilename, objectClass, changeType, serverType, isPeople, isCreateOUs, isUserUid, useShortNames, overWriteExitingOutput, toLowerCase, shortNameLength, numberOfEntrie
    ...
    {
	fillFixedArrays();
	generateLDIF(dataPath, baseDN, outPutFilename, objectClass, changeType, serverType, isPeople, isCreateOUs, isUidNaming, useShortNames, toLowerCase, overWriteExitingOutput,
		shortNameLength, numberOfEntries);
    }
    ...
    public static void generateLDIF(String dataPath, String baseDN, String outPutFilename, String[] objectClass, String changeType, String serverType, boolean isPeople,
	    boolean isCreateOUs, boolean isUserUid, boolean useShortNames, boolean toLowerCase, boolean overWriteExitingOutput, int shortNameLength, int numberOfEntries)
    {
	fillFixedArrays();
	...
	dbGen(baseDN, bufferedWriter, objectClass, serverType, serverType, isPeople, isCreateOUs, isUserUid, useShortNames, overWriteExitingOutput, shortNameLength,
		numberOfEntries);
    }
     ...
     */
    public static void dbGen(String baseDN, BufferedWriter bufferedWriter, String[] objectClass, String changeType, String serverType, boolean isPeople, boolean isCreateOUs,
	    boolean isUserUid, boolean useShortNames, boolean overWriteExitingOutput, int shortNameLength, int numberOfEntries)
    {
	...
	try
	{
		...
		for (int i = 0; i < orgUnits.size(); i++)
		{
		    ...
		    {
			bufferedWriter.write(NL + "objectClass: ndsLoginProperties" + NL + "objectClass: ndsContainerLoginProperties");
		    }
		...
		for (int userCount = 1; userCount <= numberOfEntries; userCount++)
		{
		    ...
		    if ((userCount % 10 == 0) || (userCount == numberOfEntries))
		    ...
		    String shortName = generateShortName(sn, gn, shortNameLength, true);
		    String dn = generateDn(baseDN, isUserUid, useShortNames, displayName, shortName, ou);

		    // Check to see dn already exists
		    if (dnAL.contains(dn))
		    { // Duplicate entry increment Counter and loop so we try again
			...
			if (isUserUid)
			{// we are naming as uid=
			    if (useShortNames)
			    {// we are using uid as naming so set a CN value as a shortName changed 2013-12-11 to work with AD.
				bufferedWriter.write(NL + "cn: " + shortName);
			    }
			    else
			    {// we are using uid as naming so set a CN value as a displayName
				bufferedWriter.write(NL + "cn: " + displayName);
			    }
			}
			else
			{// using CN as naming value so do not put in LDIF
			    if (useShortNames)
			    {// we are using uid as naming so set a CN value as a shortName changed 2013-12-11 to work with AD.
			...
			{
			    bufferedWriter.write(NL + "sAMAccountName: " + shortName);
			    bufferedWriter.write(NL + "userPrincipalName: " + shortName + "@" + baseDN);
			}
			bufferedWriter.write(NL + "description: This is " + gn + " " + sn + "'s description");
			bufferedWriter.write(NL + "facsimileTelephoneNumber: +1 " + ac + " " + com.willeke.utility.RandConvUtility.randInt(100, 999) + "-"
			...
			bufferedWriter.write(NL + "telephoneNumber: +1 " + ac + " " + com.willeke.utility.RandConvUtility.randInt(100, 999) + "-"
				+ com.willeke.utility.RandConvUtility.randInt(1000, 9999));
			bufferedWriter.write(NL + "title: " + getRand(titleRanks) + " " + ou + " " + getRand(positions));
			...
			bufferedWriter.write(NL + "departmentNumber: " + com.willeke.utility.RandConvUtility.randInt(1000, 9999));
			bufferedWriter.write(NL + "employeeType: " + getRand(employeeTypes));
			bufferedWriter.write(NL + "homePhone: +1 " + ac + " " + com.willeke.utility.RandConvUtility.randInt(100, 999) + "-"
				+ com.willeke.utility.RandConvUtility.randInt(1000, 9999));
			bufferedWriter.write(NL + "initials: " + gn.substring(0, 1) + sn.substring(0, 1));
			bufferedWriter.write(NL + "mobile: +1 " + ac + " " + com.willeke.utility.RandConvUtility.randInt(100, 999) + "-"
				+ com.willeke.utility.RandConvUtility.randInt(1000, 9999));
			bufferedWriter.write(NL + "pager: +1 " + ac + " " + com.willeke.utility.RandConvUtility.randInt(100, 999) + "-"
				+ com.willeke.utility.RandConvUtility.randInt(1000, 9999));
			bufferedWriter.write(NL + "roomNumber: " + com.willeke.utility.RandConvUtility.randInt(8000, 9999));
			...
			/**
			 * It is not feasible to use random managers if we are generating less than 10 entries.
			 */
			if (numberOfEntries >= 10)
			{
			    if (userCount + 1 < (numberOfEntries / orgUnits.size()))
			    { // Based on how many entries, we generate multiple managers and sec
				if (dnAL.size() > 2)
				{ // secretary
				    String testDN = getRand(dnAL);
				    if ((!(dn.equalsIgnoreCase(testDN)) && (!(mgrAL.contains(testDN)))))
				    { // if dn is not a MGR and this is not the same DN Make a secretary!
					bufferedWriter.write(NL + "secretary: " + testDN);
					if (!(secAL.contains(testDN)))
					{ // add to secretary array
					    log.debug("Making Entry a Secretary: " + dn);
					    secAL.add(dn);
					}
				    }
			    ...
			    { // We should have enough managers & Sec
		...
		log.debug("Finished Generating: " + numberOfEntries + " User Entries Output file is: " + outPutFilename);
	    } // if jRBPeople
     ...
     * @return
     */
    private static String generateDn(String baseDN, boolean isUserUid, boolean useShortNames, String displayName, String shortName, String ou)
    {
	String dn = null;
	if (useShortNames)
	{
	    if (isUserUid)
	    {// Generate dn based on shortName && uid
		dn = "uid=" + shortName + ",ou=" + ou + "," + baseDN;
	    }
	    else
	    {// Generate dn based on shortName && cn
		dn = "cn=" + shortName + ",ou=" + ou + "," + baseDN;
	    }
	}
	else
	{// Generate dn based on Full Name && cn
	    if (isUserUid)
	    {// Generate dn based on shortName && uid
	...
	else if (args.length == 13)
	{
	    ...
	    LDIFGeneration.generateLDIF(dataPath, baseDN, outPutFilename, objectClass, changeType, serverType, isPeople, isCreateOUs, isUidNaming, useShortNames,
		    overWriteExitingOutput, toLowerCase, shortNameLength, numberOfEntries);
	}
	else
	{
	    showHelp();
